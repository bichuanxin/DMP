...
object DeviceTypeRpt {
  def main(args: Array[String]): Unit = {
    ...
    val res1 = df.map(row => {
      ...
      (RptUtils.checkDeciceType(devicetype), list)
    }).reduceByKey((l1, l2) => {
      l1.zip(l2).map(tup => {
        tup._1 + tup._2
      })
